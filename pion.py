...
class Pion(Piece):
    ...
    def cases_atteignables(self, plateau: list, historique: list[tuple]):
        L = []
        i, j = self.position
        ...
        # avance simple
        if 0 <= i + direction < 8:
            if plateau[i + direction][j] is None:
                L.append((i + direction, j))
                # avance double
                if i == ligne_depart and plateau[i + 2 * direction][j] is None:
        ...
        # prise en passant
        if historique:
            depart, arrivee = historique[-1]
            x1, y1 = depart
            x2, y2 = arrivee
            piece:Piece = plateau[x2][y2]
